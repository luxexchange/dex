...
export const POOL_MANAGER_ABI = [
  ...
  // Execute a swap (marketable order against the CLOB).
  // Returns a V4 BalanceDelta packed as two signed int128 halves in one 32-byte
  // word: amount0 in the high 128 bits, amount1 in the low 128 bits. Decode with
  // unpackBalanceDelta() — NOT as a single int256 (see Go PackBalanceDelta).
  {
    type: 'function',
    name: 'swap',
    inputs: [
  ...
  // Settle the caller's own outstanding balance (flash-accounting).
  // On-chain selector keccak4("settle()") = 0x11DA60B4.
  {
    type: 'function',
    name: 'settle',
    inputs: [],
  ...
  // Settle an outstanding balance on behalf of `recipient`.
  // On-chain selector keccak4("settleFor(address)") = 0x3DD45ADB.
  // NOTE: the previous `settle(address)` shape hashed to 0x6A256B29, which the
  // precompile does not dispatch — it reverted "unknown method selector".
  {
    type: 'function',
    name: 'settleFor',
