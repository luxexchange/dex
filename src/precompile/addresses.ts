...
export const LX = {
  /**
   * LXPool (LP-9999; canonical DEX V4 receipt-settlement precompile — formerly
   * LP-9010 at 0x9010, now superseded).
   * V4 PoolManager facade over a CENTRAL LIMIT ORDER BOOK (NOT an AMM).
   * The standard V4 ABI maps onto CLOB operations on the D-Chain:
   * - initialize       → create the market
   * - swap             → settle a BLS-certified D-Chain fill receipt (in hookData)
   * - modifyLiquidity  → place (+delta) / cancel (−delta) a resting limit order
   * The C-Chain verifies the receipt cert and credits balances; it never matches.
   * Price comes from the resting D-Chain book, not a bonding curve. See LP-9999.
   */
  LX_POOL: '0x0000000000000000000000000000000000009999' as Address,
